...
# ============================================================================
# CUSTOM EXCEPTIONS FOR SPOT TRADING
# ============================================================================

class InsufficientFundsError(Exception):
    """Raised when user has insufficient cash balance"""
    pass


class InsufficientSharesError(Exception):
    """Raised when user has insufficient shares to sell"""
    pass
...
async def validate_order_requirements(
    db: Session,
    user_id: int,
    ...
    """
    Validate order requirements before placing.
    Returns validation results with availability checks.
    """
    ...
    result = {
        ...
        "estimated_fee": float(trading_service.calculate_fee(
            Decimal(str(order_data.quantity)) * 
            (Decimal(str(order_data.price)) if order_data.price else Decimal('100'))
        )),
    }
    ...
    if order_data.side == OrderSide.BUY:
        available_cash = (
            ...
            Decimal(str(portfolio.reserved_cash))
        )
        
        result["available_cash"] = float(available_cash)
        result["sufficient_funds"] = available_cash >= estimated_cost
        
        if not result["sufficient_funds"]:
            result["valid"] = False
            result["error"] = (
                f"Insufficient funds. Required: ${estimated_cost:.2f}, "
    ...
    # Check SELL order requirements
        ...
        if holding:
            available_shares = (
                ...
                Decimal(str(holding.reserved_quantity))
            )
        else:
            available_shares = Decimal('0')
        
        result["available_shares"] = float(available_shares)
        result["sufficient_shares"] = available_shares >= Decimal(str(order_data.quantity))
        
        if not result["sufficient_shares"]:
            result["valid"] = False
            result["error"] = (
                f"Insufficient shares. Required: {order_data.quantity}, "
...
async def create_order(
    ...
    Validates portfolio balance/holdings before execution.
    """
    trading_service = TradingService(db)

    try:
        # Pre-validate order requirements
        validation = await validate_order_requirements(
            db, current_user.id, order_data, trading_service
        )
        
        if not validation["valid"]:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=validation.get("error", "Order validation failed")
            )
...
async def exit_position(
    ...
    """
    Exit a position by selling all available shares of a symbol.
    ...
    try:
        ...
        # Calculate available quantity (exclude reserved shares)
        available_quantity = (
            Decimal(str(holding.quantity)) - 
            Decimal(str(holding.reserved_quantity))
        )
        ...
        if available_quantity <= 0:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="No available shares to sell. All shares are reserved in pending orders."
            )
        
        # Create sell order for full available position
        order_data = OrderCreate(
        ...
        if not order:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Failed to create exit order"
            )
