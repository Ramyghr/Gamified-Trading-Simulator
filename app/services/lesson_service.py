...
class LessonService:
    ...
    def submit_quiz(
        ...
        # Evaluate answers
        total_points = sum(q.points if q.points else 10 for q in questions)
        earned_points = 0
        results = []
        ...
        for question in questions:
            ...
            # ✅ Compare as strings to handle mixed DB types (int or str)
            is_correct = (
                str(user_answer).strip() == str(question.correct_answer).strip()
                if user_answer is not None and question.correct_answer is not None
                else False
            )

            points_for_question = question.points if question.points else 10
            if is_correct:
