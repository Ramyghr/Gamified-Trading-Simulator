...
class MarketDataAggregator:
    ...
    async def get_quote(self, symbol: str, asset_class: Optional[AssetClass] = None) -> QuoteResponse:
        ...
        for provider in providers:
            try:
                ...
                if quote.close > 0:  # Only cache valid data
                    ...
                    logger.warning(f"Provider {provider.name} returned zero price for {symbol}")
        ...
        # Return a fallback quote with zero values (will pass validation with ge=0)
        return QuoteResponse(
            ...
            provider=DataProvider.FALLBACK
        )
    ...
    def _detect_asset_class(self, symbol: str) -> AssetClass:
        ...
        # --- Crypto detection ---
        crypto_keywords = ["BTC", "ETH", "BNB", "SOL", "ADA", "XRP", "DOGE", "USDT", "USDC"]
        if any(k in symbol_upper for k in crypto_keywords):
            return AssetClass.CRYPTO

        # --- Forex detection ---
        if "/" in symbol and len(symbol) == 6:  # e.g. EUR/USD
