...
class AlphaVantageProvider(BaseMarketDataProvider):
    ...
    async def get_quote(self, symbol: str) -> QuoteResponse:
        """Get real-time quote with better error handling"""
        endpoint = "/query"
        params = {
        ...
        try:
            ...
            if "Global Quote" not in data:
                raise Exception(f"No quote data for {symbol} - response: {data}")
            ...
            prev_close = float(quote_data.get("08. previous close", close_price))  # Fallback to current price
        ...
        for timestamp_str, candle_data in list(time_series.items())[:limit]:
            ...
            candles.append(
                CandleResponse(
                    symbol=symbol,
                    timeframe=timeframe,
                    open=float(candle_data.get("1. open", candle_data.get("1. Open", 0))),
                    high=float(candle_data.get("2. high", candle_data.get("2. High", 0))),
                    low=float(candle_data.get("3. low", candle_data.get("3. Low", 0))),
                    close=float(candle_data.get("4. close", candle_data.get("4. Close", 0))),
                    volume=float(candle_data.get("5. volume", candle_data.get("5. Volume", 0))),
                    timestamp=timestamp,
                    provider=DataProvider.ALPHA_VANTAGE
                )
            )
